...
<div class="shell" class:menu-open={menuOpen}>
	...
	<nav id="mobile-menu" class="mobile-sheet" aria-label="Mobile" inert={!menuOpen}>
		{#each navLinks as link (link.href)}
			<a href={link.href} onclick={close}>{link.label}</a>
		{/each}
		<a href={LINKS.github} target="_blank" rel="noopener noreferrer" onclick={close}>
			<GithubMark size={15} />
			GitHub
		</a>
		<a href={LINKS.license} target="_blank" rel="noopener noreferrer" onclick={close}>
			{LICENSE} licence
		</a>
	...
	<footer class="site-footer">
		...
		<div class="footer-identity">
			...
			<nav class="footer-links" aria-label="Footer">
				...
				<a href={LINKS.license} target="_blank" rel="noopener noreferrer">{LICENSE} licence</a>
			</nav>
		</div>

		<div class="footer-base">
			<span>© {year} {COMPANY.name}</span>
			<span class="footer-lifecycle">specify → review → approve → implement</span>
		</div>
	</footer>
</div>
